...
{
    scene->camera = add_vec3(scene->camera, move);
    scene->cam_dir = add_vec3(scene->cam_dir, move);
    // printf("2. %p\n", scene);
    frame = 0;
    free(sum);
    sum = NULL;
...
int listen_on_key(int code, Win *win)
{
    ...
    case ROTATE_RIGHT:
    {
        ...
        free(sum);
        sum = NULL;
        scene->cam_dir = rotate(degrees_to_radians(angle), scene->cam_dir, 'y');
        break;
    }
    case ROTATE_LEFT:
    {
