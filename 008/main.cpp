...
double hit_triangle(Point v0, Point v1, Point v2, Ray ray, float t_min, float t_max)
{
...
Color ray_color(const Ray &ray, int depth, World &world)
{
    ...
    {
        double t = hit_triangle(world.triangles[i].v0, world.triangles[i].v1, world.triangles[i].v2, ray, 0.001, closest);
...
inline Triangle *add_triangle(World &world, Vector v0, Vector v1, Vector v2, Material *m)
{
    Triangle *t = &world.triangles[world.triangle_count++];
...
int main(void)
{
    ...
    Point lookfrom = Point(0, 0, 4); // Point camera is looking from
   ...
   // add_sphere(world, Point(0, -1000, 0), 1000, m);
    ...
    add_triangle(world, Vector(0, 0, 0), Vector(0, 1, 0), Vector(-1, 0, 0), m);
