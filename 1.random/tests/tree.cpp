...
Obj new_cylinder(Vec3 center, float radius, float height, Vec3 normal, Color color, Mat mat)
{
    ...
    Vec3 v = {1, 1, 1};
    obj.normal = unit_vector(normal);
    obj.u = unit_vector(cross_(obj.normal, v));
    obj.v = unit_vector(cross_(obj.normal, obj.u));
...
{
#if 1
    // check if perp to normal
    Vec3 org = ray.org - cylin.center;
    float a = pow2(dot(ray.dir, cylin.u)) + pow2(dot(ray.dir, cylin.v));
    float b = 2 * (dot(org, cylin.u) * dot(ray.dir, cylin.u) + dot(org, cylin.v) * dot(ray.dir, cylin.v));
    float c = pow2(dot(org, cylin.u)) + pow2(dot(org, cylin.v)) - pow2(cylin.radius);
    float delta = b * b - 4 * a * c;
    if (delta < 0)
        return -1.0;
    delta = sqrt(delta);
    float sol = (-b - delta) / (2 * a);
    if (sol <= min || sol >= max)
        sol = (-b + delta) / (2 * a);
    if (sol <= min || sol >= max)
        return -1.0;
    float val = fabsf(dot(point_at(ray, sol) - cylin.center, cylin.normal));
    if (val < cylin.height / 2)
        return (sol);

    // float t = dot(cylin.center, cylin.normal) - dot(ray.org, cylin.normal);
    // float div = dot(ray.dir, cylin.normal);
    // if (div == 0)
    //     return -1.0;
    // t /= div;
    // Vec3 p = point_at(ray, t);
    // Vec3 pc = p - cylin.center;
    // if (length(pc) <= cylin.radius)
    //     return t;
    ...
    if (length(pc) <= cylin.radius)
        return t;
    return -1.0;
    float a = dot(cylin.normal, cylin.normal);
    float b = -2 * dot(unit_vector(p - cylin.center), cylin.normal);
    float c = dot(p - cylin.center, p - cylin.center) - pow(cylin.radius, 2);
...
float hit_object(Obj *obj, Ray ray, float closest)
{
    float x = -1.0;
    if (obj->type == sphere_)
        x = hit_sphere(*obj, ray, ZERO, closest);
    else if (obj->type == hemisphere_)
        x = hit_hemisphere(*obj, ray, ZERO, closest);
    else if (obj->type == cylinder_)
        x = hit_cylinder(*obj, ray, ZERO, closest);
    else if (obj->type == plan_)
        x = hit_plan(*obj, ray, ZERO, closest);
    else if (obj->type == triangle_)
        x = hit_triangle(*obj, ray, ZERO, closest);
    else if (obj->type == rectangle_)
        x = hit_rectangle(*obj, ray, ZERO, closest);
    return x;
}

Color ray_color_recursive(Obj *obj, Ray ray, int depth, float closest)
{
    if (obj == NULL || depth <= 0)
        return BACKGROUND(0.5 * (unit_vector(ray.dir).y + 1.0));

    Color light = {};
    Color attenuation = {1, 1, 1};

    float x = hit_object(obj, ray, closest);
    ...
    Color left_color = ray_color_recursive(obj->left, ray, depth - 1, closest);
    Color right_color = ray_color_recursive(obj->right, ray, depth - 1, closest);

    return light + left_color + right_color;
}
...
Color ray_color(Win *win, Ray ray, int depth)
{
    ...
    return ray_color_recursive(win->scene.objects, ray, depth, closest);
}
extern int upaxis;
        ...
        for (int h = y_start; h < y_start + height; h++)
        {
            for (int w = x_start; w < x_start + width; w++)
            {
                ...
                for (int i = 0; i < rays_per_pixel; i++)
                {
                    ...
                    pixel = pixel + ray_color(win, ray, 50);
#if FRAMES_LEN
                }
...
void printvalues(Obj *root)
{
    if (root != NULL)
    {
        printvalues(root->left);
        std::cout << "(" << root->center.x << " , " << root->center.y << " , " << root->center.z << ") " << std::endl;
        printvalues(root->right);
    }
}

int dist(Vec3 center)
{
    return (int)sqrt(pow2(center.x) + pow2(center.y) + pow2(center.z));
}

Obj *insert(Obj *node, Vec3 center, float radius, Color color, Mat mat)
{
    if (node == NULL)
    {
        Obj *obj = (Obj *)calloc(1, sizeof(Obj));
        *obj = new_sphere(center, radius, color, mat);
        return obj;
    }

    int value1 = dist(node->center);
    int value2 = dist(center);

    if (value1 > value2)
        node->left = insert(node->left, center, radius, color, mat);
    else if (value1 <= value2)
        node->right = insert(node->right, center, radius, color, mat);
    return node;
}
...
void add_objects(Win *win)
{
    ...
    win->scene.objects = insert(win->scene.objects, center, rad, colors[i], mat);
    i++;
    while (spheres[i].mat)
    {
