...
float hit_triangle(Ray *ray, Vec3 p1, Vec3 p2, Vec3 p3, Vec3 normal, float max)
{
...
float hit_rectangle(Ray *ray, Vec3 p1, Vec3 p2, Vec3 p3, Vec3 normal, float max)
{
    float t = dot(normal, (p1 - ray->org));
    float div = dot(normal, ray->dir);
    if (fabsf(div) <= ZERO)
...
#if 1
float hit_cylinder(Ray *ray, Vec3 center, float radius, float height, Vec3 normal, float max, Vec3 *hit_normal)
{
    Vec3 ran = {0, 0, 1};
    ...
    float d1 = hit_plan(ray, normal, -dot(c1, normal), ZERO, max);
    if (d1 > 0)
    {
    ...
    float d2 = hit_plan(ray, -1 * normal, -dot(c2, -1 * normal), ZERO, max);
    if (d2 > 0)
    {
...
void add_objects(Win *win)
{
    ...
    while (i < sizeof(plans) / sizeof(*plans))
    {
        ...
        win->scene.objects[win->scene.pos++] = new_plan(normal, d, color, Abs_);
        i++;
    }
#endif
    ...
    while (i < sizeof(cylins) / sizeof(*cylins))
    {
        ...
        win->scene.objects[win->scene.pos++] = new_cylinder(center, radius, height, normal, color, Abs_);
        i++;
    }
#endif
}
