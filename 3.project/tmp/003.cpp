...
{
    // std::cout << "call hit cone" << std::endl;
    // float d = dot((ray->org - center), normal);
    // if (d > height || d < 0)
    //     return -1.0;
#if 0
    Vec3 ran = {0, 0, 1};
    ...
    return x;
#else
    Vec3 C = center + height * unit_vector(normal);
    Vec3 N = -1 * unit_vector(normal);
    Vec3 CR = ray->org - C;
    float angle = degrees_to_radians(30);
    
    float a = pow2(dot(ray->dir, N)) - pow2(cos(angle));
    float b = 2 * (dot(CR, N) * dot(ray->dir, N) - dot(CR, ray->dir) * pow2(cos(angle)));
    float c = pow2(dot(CR, N)) - dot(CR, CR) * pow2(cos(angle));
    float delta = b * b - 4 * a * c;
    if (delta < 0)
        return -1.0;
    delta = sqrt(delta);
...
Color ray_color(Win *win, Ray *ray, int max_depth)
{
    ...
    for (int depth = 0; depth < max_depth; depth++)
    {
        ...
        if (hit_index != -1)
        {
            ...
            // lightness = lightness + darkness * obj->color;
            darkness = darkness * obj->color;
#endif
        }
        else
...
void add_objects(Win *win)
{
    ...
    //     i++;
    // }
    win->scene.objects[win->scene.pos++] = new_sphere((Vec3){0, 0, -6}, 1.8, (Color){1, 0, 0}, Abs_);
    win->scene.objects[win->scene.pos++] = new_sphere((Vec3){-4, 0, -6}, 1.8, (Color){1, 0, 0}, Abs_);
