...
void	check_solution(t_equ *equ, t_ray *ray, t_obj cylin, float max)
{
	...
	if (x1 > ZERO && x1 < max)
		equ->sol = x1;
	if (x2 > ZERO && x2 < max && (equ->sol < 0 || x2 < equ->sol))
		equ->sol = x2;
	p = point_at(ray, equ->sol);
	cal = fabsf(dot(cylin.normal, calc(p, '-',
					cylin.center)));
	if (cal > cylin.height / 2)
		equ->sol = -1.0;
	if (equ->sol > 0)
		*(equ->hit_normal) = calc(p, '-', calc(scale(cylin.normal, '*',
						dot(calc(p, '-', cylin.center), cylin.normal)),
					'+', cylin.center));
}
