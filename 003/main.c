...
Object new_sphere(Coor center, double radius, Color color, Mater mat)
{
    Object obj;
...
Color ray_color(Ray ray, int depth)
{
    ...
    {
        double t;
        if (World[i].type == Sphere_)
            t = hit_sphere(World[i].sphere, ray, 0.0001, closest);
        if (t > 0.0 && t < closest)
        {
            hit_index = i;
            closest = t;
    ...
    {
        Sphere sphere;
        if (World[hit_index].type == Sphere_)
            sphere = World[hit_index].sphere;
    ...
    double a = 0.5 * (unit_vector(ray.dir).y + 1.0); // value in [0,1]
    double r = (1.0 - a) + a * 0.3;
...
int main()
{
    World[pos++] = new_sphere(new (-1, 0, 2), 0.45, RGB(48, 134, 111), Reflec_);
    World[pos++] = new_sphere(new (1, 0, 2), 0.45, RGB(108, 134, 48), Reflec_);
    World[pos++] = new_sphere(new (0, 0, 2), 0.45, RGB(60, 33, 7), Reflec_);
    World[pos++] = new_sphere(new (0, -100.5, 1), 100, RGB(255.999, 0, 0), Absorb_);
